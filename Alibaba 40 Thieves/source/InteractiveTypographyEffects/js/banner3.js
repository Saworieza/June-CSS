function Banner(){
	...
	var prepare 		= function() {
		...
		bgContext.font = "250px 'Salsa'";
		
		//Fill the keyword text onto the canvas.
		bgContext.fillText(keyword, ( canvasW / 2 ) - ( Math.round( bgContext.measureText(keyword).width/2 ) ) , 240 );
	...
	var draw 			= function() {
		...
		var dx, dy, sqrDist,
 		...
 		for( var i = 0, len = particles.length; i < len; ++i ) {
			...
			( isDrawing ) ? scale = Math.max( Math.min( 18 - ( sqrDist / 10 ), 10 ), 1 ) : scale = 1;
